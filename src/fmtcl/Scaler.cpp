	...
	for (int y = y_dst_beg; y < y_dst_end; ++y)
	{
		...
		else
		{
			...
			for (int x = 0; x < w8; x += 8)
			{
				...
				process_vect_flt_sse <SRC, false> (
...
void	Scaler::process_vect_flt_sse (__m128 &sum0, __m128 &sum1, int kernel_size, const float *coef_base_ptr, typename SRC::PtrConst::Type pix_ptr, const __m128i &zero, int src_stride, const __m128 &add_cst, int len)
{
	...
	for (int k = 0; k < kernel_size; ++k)
	{
		...
		__m128         src0;
		__m128         src1;
		ReadWrapperFltSse <SRC, PF>::read (pix_ptr, src0, src1, zero, len);
...
template <class SRC, bool PF>
void	Scaler::ReadWrapperFltSse <SRC, PF>::read (typename SRC::PtrConst::Type ptr, __m128 &src0, __m128 &src1, const __m128i &zero, int /*len*/)
{
	SRC::read_flt (ptr, src0, src1, zero);
}

template <class SRC>
void	Scaler::ReadWrapperFltSse <SRC, true>::read (typename SRC::PtrConst::Type ptr, __m128 &src0, __m128 &src1, const __m128i &zero, int len)
{
		...
		else
		{
			...
			for (int x = 0; x < w8; x += 8)
			{
				...
				const __m128i  val = process_vect_int_sse2 <DST, DB, SRC, SB> (
					...
					pix_ptr, zero, src_stride, sign_bit
				);
			...
			if (w7 > 0)
			{
				...
				const __m128i  val = process_vect_int_sse2_partial <DST, DB, SRC, SB> (
					add_cst, kernel_size, coef_base_ptr,
...
template <class DST, int DB, class SRC, int SB>
__m128i	Scaler::process_vect_int_sse2_partial (const __m128i &add_cst, int kernel_size, const __m128i coef_base_ptr [], typename SRC::PtrConst::Type pix_ptr, const __m128i &zero, int src_stride, const __m128i &sign_bit, int len)
{
	typedef typename SRC::template S16 <false, (SB == 16)> SrcS16R;
	...
	{
		__m128i        coef = _mm_load_si128 (coef_base_ptr + k);
		__m128i        src  = SrcS16R::read_partial (pix_ptr, zero, sign_bit, len);
		if (DB > SB)
		{
			src = _mm_slli_epi16 (src, DB - SB);
			if (DB == 16)
	...
	{
		const __m128i  coef = _mm_load_si128 (coef_base_ptr + k);
		const __m128i  src  = SrcS16R::read_partial (pix_ptr, zero, sign_bit, len);
		fstb::ToolsSse2::mac_s16_s16_s32 (sum0, sum1, src, coef);
